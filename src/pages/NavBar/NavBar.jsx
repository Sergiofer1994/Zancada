import { useNavigate, useLocation } from 'react-router-dom'
import { Home, List, Map, BarChart3, User } from 'lucide-react'
import './NavBar.css'

function NavBar() {
  const navigate = useNavigate()
  const location = useLocation()

  const isActive = (path) => location.pathname === path

  return (
    <nav role="navigation" aria-label="Navegación principal">
      <button
        type="button"
        className={isActive('/home') ? 'nav-item active' : 'nav-item'}
        onClick={() => navigate('/home')}
        aria-label="Ir a inicio"
        aria-current={isActive('/home') ? 'page' : undefined}
      >
        <Home className="nav-icon" size={20} aria-hidden="true" />
        <span className="nav-label">Inicio</span>
      </button>

      <button
        type="button"
        className={isActive('/history') ? 'nav-item active' : 'nav-item'}
        onClick={() => navigate('/history')}
        aria-label="Ir a historial"
        aria-current={isActive('/history') ? 'page' : undefined}
      >
        <List className="nav-icon" size={20} aria-hidden="true" />
        <span className="nav-label">Historial</span>
      </button>

      <button
        type="button"
        className={isActive('/routes') ? 'nav-item active' : 'nav-item'}
        onClick={() => navigate('/routes')}
        aria-label="Ir a rutas"
        aria-current={isActive('/routes') ? 'page' : undefined}
      >
        <Map className="nav-icon" size={20} aria-hidden="true" />
        <span className="nav-label">Rutas</span>
      </button>

      <button
        type="button"
        className={isActive('/stats') ? 'nav-item active' : 'nav-item'}
        onClick={() => navigate('/stats')}
        aria-label="Ir a estadísticas"
        aria-current={isActive('/stats') ? 'page' : undefined}
      >
        <BarChart3 className="nav-icon" size={20} aria-hidden="true" />
        <span className="nav-label">Estadísticas</span>
      </button>

      <button
        type="button"
        className={isActive('/profile') ? 'nav-item active' : 'nav-item'}
        onClick={() => navigate('/profile')}
        aria-label="Ir a perfil"
        aria-current={isActive('/profile') ? 'page' : undefined}
      >
        <User className="nav-icon" size={20} aria-hidden="true" />
        <span className="nav-label">Perfil</span>
      </button>
    </nav>
  )
}

export default NavBar